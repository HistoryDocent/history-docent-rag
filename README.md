...
| voice provider 전략 | 무료 로컬 STT/TTS 우선. STT는 `faster-whisper small CUDA`를 현재 demo evidence 기준 primary 후보로 고정했다. TTS는 MeloTTS Windows `eunjeon` blocker, Piper Korean voice 부재, `sherpa-onnx` Supertonic 3 Korean 자동 proxy 4/5 통과, 사람 청취 점수 0/30 때문에 final provider 없이 차단한다. Azure/Google/AWS는 optional paid comparison only |
