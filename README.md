...
현재 단계는 canonical source를 `History_Docent`로 고정하고, 원본 PDF와 Upstage Parser 산출물의 `source_inventory` gate를 구현하는 것이다.

`source_inventory`가 통과한 뒤 `data_manifest`와 `normalized_blocks` schema 구현으로 이동한다.
