...
| metric | value |
| --- | ---: |
| summarized_stage_count | 26 |
| adopted_or_implemented_count | 14 |
...
- `router_message`: query type classifier baseline, router skeleton, API dry-run, relationship guard, guarded route candidate, active routing 판단 계획, active route shadow evaluation은 구현됐지만 production routing과 locked 성능 개선은 아직 없다.
...
- `external_audit`: 실패 원인표와 targeted audit으로 청킹 재실험 범위를 닫았고 HyDE larger live 비교와 active route shadow evaluation도 완료했다. Recall@5만 보고 채택하지 않고 MRR, nDCG@5, latency, active route safety gate를 같이 본 판단이 타당하다.
