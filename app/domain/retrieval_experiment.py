...
def build_qualitative_assessment(
    *,
    ...
    return {
        "harness_scope": (
            f"공통 schema로 평가한 method는 {methods}다. Dense/Hybrid 구현은 아직 포함하지 않았다."
        ),
        "baseline_reproduction": baseline_text,
        "comparison_status": (
            "현재 delta는 harness 검증용이다. 성능 개선 주장이 아니라 비교 형식 고정이다."
        ),
        "public_policy": (
            "public result와 report에는 rank, id, score, metric만 저장하고 검색 본문은 저장하지 않는다."
        ),
        "next_step": (
            "Dense retriever와 Hybrid retriever를 같은 report에 추가한 뒤 query type별 delta를 비교한다."
        ),
