...
from pipelines.run_raptor_lite_input_only import (
    BASELINE_STRATEGY_ID,
    RAPTOR_LITE_INPUT_ONLY_REPORT_VERSION,
    collect_raptor_lite_input_only_failures,
    load_baseline_results_from_public_rows,
    run_raptor_lite_input_only,
)


def test_raptor_lite_input_only_writes_public_safe_report(tmp_path: Path) -> None:
    report_path = tmp_path / "raptor_lite_input_only_report.md"
    rows_path = tmp_path / "raptor_lite_input_only_rows.jsonl"
    items = _eval_items()
    ...
    report = run_raptor_lite_input_only(
        ...
        expected_target_dev_query_count=2,
    )
    markdown = report_path.read_text(encoding="utf-8")
    rows = [json.loads(line) for line in rows_path.read_text(encoding="utf-8").splitlines()]

    assert report.report_version == RAPTOR_LITE_INPUT_ONLY_REPORT_VERSION
    assert report.summary.target_dev_query_count == 2
    assert report.summary.expected_target_dev_query_count == 2
    assert report.summary.query_type_counts == {"overview": 1, "place_story": 1}
    ...
    assert collect_raptor_lite_input_only_failures(report) == []
    assert {row.strategy_id for row in report.metric_rows} == {
        BASELINE_STRATEGY_ID,
        "raptor_lite_parent_summary_v1",
        "raptor_lite_summary_node_v1",
    }
    assert {row.query_scope for row in report.metric_rows} == {
        "combined",
        "overview",
        "place_story",
    }
...
def test_load_baseline_results_from_public_rows_filters_target_items(
    tmp_path: Path,
) -> None:
    ...
    assert results[0].query_id == "q-overview-1"
    assert results[0].query_type == "overview"
    assert results[0].latency_ms == 12.5
    assert [candidate.child_id for candidate in results[0].candidates] == [
        "child-palace"
    ]
...
def _documents() -> list[RetrievalDocument]:
    return [
        _document(
            ...
            parent_id="parent-palace",
            doc_id="doc-palace",
            search_text="정도전 한양 천도 경복궁 궁궐 정치 배경",
        ),
        _document(
            child_id="child-king",
            parent_id="parent-king",
            doc_id="doc-king",
            search_text="세종 집현전 훈민정음 제도 개혁 문화",
        ),
        _document(
            child_id="child-market",
            parent_id="parent-market",
            doc_id="doc-market",
            search_text="종로 시장 상업 거리 생활 변화",
        ),
    ]
...
def _eval_items() -> list[RetrievalEvalItem]:
    return [
        ...
        RetrievalEvalItem.model_validate(
            _eval_item_payload(
                query_id="q-place-story-1",
                query_type="place_story",
                query_text="세종과 집현전 이야기를 들려줄 근거",
                child_id="child-king",
                parent_id="parent-king",
                doc_id="doc-king",
            )
