...
from pipelines.voice_stt_tts_azure_smoke_execution import (
    NEXT_WORK_ID,
    WORK_ID,
    collect_azure_smoke_execution_failures,
    run_voice_stt_tts_azure_smoke_execution,
)


DOC_PATH = Path("docs/VOICE_STT_TTS_AZURE_SMOKE_EXECUTION.md")
REPORT_PATH = Path("evals/reports/voice_stt_tts_azure_smoke_execution_report.md")
...
REQUIRED_LINKS = (
    "docs/VOICE_STT_TTS_AZURE_SMOKE_EXECUTION.md",
    "evals/reports/voice_stt_tts_azure_smoke_execution_report.md",
)
PUBLIC_SCAN_PATHS = (
    README_PATH,
    DOC_PATH,
...
def test_azure_smoke_execution_missing_credentials_is_public_safe(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    _clear_azure_env(monkeypatch)

    report = run_voice_stt_tts_azure_smoke_execution(
        env_path=tmp_path / "missing.env",
        doc_path=tmp_path / "VOICE_STT_TTS_AZURE_SMOKE_EXECUTION.md",
        report_path=tmp_path / "voice_stt_tts_azure_smoke_execution_report.md",
        result_rows_path=tmp_path / "voice_stt_tts_azure_smoke_execution_rows.jsonl",
    )

    assert collect_azure_smoke_execution_failures(report) == []
    ...
    rows = [
        json.loads(line)
        for line in (tmp_path / "voice_stt_tts_azure_smoke_execution_rows.jsonl")
...
def test_azure_smoke_execution_ready_credentials_still_blocks_without_source_and_user(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    _clear_azure_env(monkeypatch)
...
def test_azure_smoke_execution_execute_flag_is_blocked_without_gate(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    _clear_azure_env(monkeypatch)
    ...
    with pytest.raises(ValueError, match="blocked by execution gate"):
        run_voice_stt_tts_azure_smoke_execution(
            ...
            report_path=tmp_path / "report.md",
            result_rows_path=tmp_path / "rows.jsonl",
            execute_azure_provider=True,
        )


def test_azure_smoke_execution_docs_record_zero_call_result() -> None:
...
def test_azure_smoke_execution_is_registered_and_public_safe() -> None:
    ...
    assert "- [x] optional voice STT/TTS Azure smoke execution" in todo
...
def test_azure_smoke_execution_keeps_forbidden_claims_forbidden() -> None:
    doc = DOC_PATH.read_text(encoding="utf-8")
