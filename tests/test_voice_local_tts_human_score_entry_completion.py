...
import pipelines.voice_local_tts_human_score_entry_completion as completion


DOC_PATH = Path("docs/VOICE_LOCAL_TTS_HUMAN_SCORE_ENTRY_COMPLETION.md")
REPORT_PATH = Path("evals/reports/voice_local_tts_human_score_entry_completion_report.md")
...
REQUIRED_LINKS = (
    "docs/VOICE_LOCAL_TTS_HUMAN_SCORE_ENTRY_COMPLETION.md",
    "evals/reports/voice_local_tts_human_score_entry_completion_report.md",
)
PUBLIC_SCAN_PATHS = (
    README_PATH,
    DOC_PATH,
...
def _write_completed_private_scores(path: Path) -> None:
    ...
    with path.open("w", encoding="utf-8", newline="\n") as output:
        for script_index in range(1, 6):
            ...
            for criterion in completion.RUBRIC:
                output.write(
                    json.dumps(
                        {
                            "provider_candidate_id": completion.PROVIDER_CANDIDATE_ID,
                            "script_id": script_id,
...
def test_human_score_completion_runner_blocks_missing_manual_scores(
    tmp_path: Path,
) -> None:
    audio_dir = tmp_path / "audio"
    _write_fixture_audio_files(audio_dir)

    report = completion.run_voice_local_tts_human_score_entry_completion(
        private_audio_dir=audio_dir,
        private_score_input_path=tmp_path / "missing_scores.jsonl",
        doc_path=tmp_path / "VOICE_LOCAL_TTS_HUMAN_SCORE_ENTRY_COMPLETION.md",
        report_path=tmp_path / "voice_local_tts_human_score_entry_completion_report.md",
        result_rows_path=tmp_path / "voice_local_tts_human_score_entry_completion_rows.jsonl",
    )

    assert completion.collect_completion_failures(report) == []
    assert completion.collect_completion_blockers(report) == [
    ...
    assert report.summary.private_score_input_row_count == 0
    assert report.summary.valid_private_score_row_count == 0
    ...
    assert report.summary.completed_script_rate == 0.0
    ...
    assert report.summary.score_completion_decision == "blocked_missing_human_scores"

    public_rows = [
        json.loads(line)
        for line in (tmp_path / "voice_local_tts_human_score_entry_completion_rows.jsonl")
...
def test_human_score_completion_runner_accepts_completed_private_scores(
    tmp_path: Path,
) -> None:
    audio_dir = tmp_path / "audio"
    ...
    assert (
        report.summary.score_completion_decision
...
def test_human_score_completion_docs_record_blocked_state() -> None:
    ...
    assert completion.WORK_ID in doc
    assert completion.WORK_ID in report
...
def test_human_score_completion_keeps_forbidden_claims_forbidden() -> None:
    doc = DOC_PATH.read_text(encoding="utf-8")
