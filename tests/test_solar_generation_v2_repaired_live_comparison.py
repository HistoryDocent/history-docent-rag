...
from pipelines.run_solar_generation_v2_repaired_live_comparison import (
    SOLAR_GENERATION_V2_REPAIRED_LIVE_COMPARISON_REPORT_VERSION,
    collect_solar_generation_v2_repaired_live_comparison_failures,
    run_solar_generation_v2_repaired_live_comparison,
)
...
def test_solar_generation_v2_repaired_live_comparison_routes_and_public_safe(
    tmp_path: Path,
) -> None:
    dataset_path = tmp_path / "retrieval_eval_dev.jsonl"
    chunks_path = tmp_path / "parent_child_chunks.json"
    report_path = tmp_path / "solar_generation_v2_repaired_live_comparison_report.md"
    rows_path = tmp_path / "solar_generation_v2_repaired_live_results.jsonl"
    ...
    baseline_provider = CountingDraftProvider(
        ...
        provider_config_id="solar-repaired-live-v1-test",
    )
    repaired_provider = CountingDraftProvider(
        ...
        provider_config_id="solar-repaired-live-v2-test",
    )
    ...
    report = run_solar_generation_v2_repaired_live_comparison(
        ...
        per_query_type=1,
        query_types=("place_fact", "place_story", "no_answer"),
        retrieval_backend=StaticRetrievalBackend(),
        baseline_draft_provider=baseline_provider,
        repaired_draft_provider=repaired_provider,
    )
    markdown = report_path.read_text(encoding="utf-8")
    rows = [json.loads(line) for line in rows_path.read_text(encoding="utf-8").splitlines()]
    route_by_query_id = {row.query_id: row for row in report.route_rows}

    assert report.report_version == SOLAR_GENERATION_V2_REPAIRED_LIVE_COMPARISON_REPORT_VERSION
    assert len(baseline_provider.requests) == 2
    assert len(repaired_provider.requests) == 1
    assert raw_chunk_text in baseline_provider.requests[0].evidence_context
    assert raw_chunk_text in repaired_provider.requests[0].evidence_context
    assert report.baseline_report.summary.eval_count == 3
    assert report.repaired_report.summary.eval_count == 3
    assert report.live_call_summary.baseline_live_call_count == 2
    assert report.live_call_summary.repaired_candidate_live_call_count == 1
    assert report.live_call_summary.expected_total_live_call_count == 3
    assert report.live_call_summary.actual_total_solar_call_count == 3
    assert report.live_call_summary.no_answer_live_call_count == 0
    assert route_by_query_id["q-dev-place-story-001"].route_decision == "use_v1_fallback"
    assert route_by_query_id["q-dev-place-story-001"].fallback_reused_baseline_answer is True
    assert route_by_query_id["q-dev-place-story-001"].actual_repaired_solar_call_count == 0
    assert route_by_query_id["q-dev-no-answer-001"].route_decision == "abstain_no_live_call"
    assert collect_solar_generation_v2_repaired_live_comparison_failures(report) == []
