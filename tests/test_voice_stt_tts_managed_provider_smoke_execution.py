...
from pipelines.voice_stt_tts_managed_provider_smoke_execution import (
    EXPLICIT_AZURE_ACTUAL_CALL_APPROVAL_PHRASE,
    NEXT_WORK_ID,
    WORK_ID,
    collect_managed_provider_smoke_execution_failures,
    run_voice_stt_tts_managed_provider_smoke_execution,
)


DOC_PATH = Path("docs/VOICE_STT_TTS_MANAGED_PROVIDER_SMOKE_EXECUTION.md")
REPORT_PATH = Path(
    "evals/reports/voice_stt_tts_managed_provider_smoke_execution_report.md",
)
...
REQUIRED_LINKS = (
    "docs/VOICE_STT_TTS_MANAGED_PROVIDER_SMOKE_EXECUTION.md",
    "evals/reports/voice_stt_tts_managed_provider_smoke_execution_report.md",
)
PUBLIC_SCAN_PATHS = (
    README_PATH,
...
def test_managed_provider_smoke_execution_missing_credentials_is_public_safe(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    _clear_azure_env(monkeypatch)

    report = run_voice_stt_tts_managed_provider_smoke_execution(
    ...
    assert collect_managed_provider_smoke_execution_failures(report) == []
    ...
    assert report.summary.planned_stt_call_count == 3
    assert report.summary.planned_tts_call_count == 3
    assert report.summary.call_cap_enforced is True
    assert report.summary.azure_credential_ready is False
    assert report.summary.managed_provider_execution_ready is False
    assert report.summary.managed_provider_execution_approved is False
    assert report.summary.execution_decision == "blocked_missing_azure_credentials"
...
def test_managed_provider_smoke_execution_ready_credentials_still_block_without_source(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    _clear_azure_env(monkeypatch)
    ...
    assert report.summary.managed_provider_execution_ready is False
    assert report.summary.managed_provider_execution_approved is False
    assert report.summary.execution_decision == "blocked_source_recheck_incomplete"
    ...
    assert "fixture-azure-region-present" not in public_text


def test_managed_provider_smoke_execution_can_reach_ready_state_without_calls(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    _clear_azure_env(monkeypatch)
    ...
    assert collect_managed_provider_smoke_execution_failures(report) == []
    assert report.summary.managed_provider_execution_ready is True
    assert report.summary.managed_provider_execution_approved is False
    assert report.summary.execution_decision == "ready_for_actual_azure_smoke_execution"
    assert report.summary.managed_provider_api_call_count == 0
    assert report.summary.external_audio_transmission_count == 0
    assert report.summary.live_stt_call_count == 0
    assert report.summary.live_tts_call_count == 0
...
def test_managed_provider_smoke_execution_execute_flag_requires_phrase(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    _clear_azure_env(monkeypatch)
    ...
    with pytest.raises(ValueError, match="blocked by gate"):
        run_voice_stt_tts_managed_provider_smoke_execution(
            doc_path=tmp_path / "doc.md",
            report_path=tmp_path / "report.md",
            result_rows_path=tmp_path / "rows.jsonl",
...
def test_managed_provider_smoke_execution_docs_record_current_zero_call_result() -> None:
    ...
    assert "managed_provider_execution_approved` | `false`" in doc
    assert "azure_credential_ready | false" in report
    assert "managed_provider_execution_approved | false" in report
    assert "execution_decision | `blocked_missing_azure_credentials`" in report
...
def test_managed_provider_smoke_execution_is_registered_and_public_safe() -> None:
    ...
    assert "- [x] optional voice STT/TTS managed provider smoke execution gate" in todo
    ...
    assert "execution_decision=blocked_missing_azure_credentials" in ledger
...
def test_managed_provider_smoke_execution_keeps_forbidden_claims_forbidden() -> None:
    doc = DOC_PATH.read_text(encoding="utf-8")
