...
import pipelines.voice_local_tts_human_score_entry as entry


DOC_PATH = Path("docs/VOICE_LOCAL_TTS_HUMAN_SCORE_ENTRY.md")
REPORT_PATH = Path("evals/reports/voice_local_tts_human_score_entry_report.md")
...
REQUIRED_LINKS = (
    "docs/VOICE_LOCAL_TTS_HUMAN_SCORE_ENTRY.md",
    "evals/reports/voice_local_tts_human_score_entry_report.md",
)
PUBLIC_SCAN_PATHS = (
    README_PATH,
    DOC_PATH,
                    ...
                    json.dumps(
                        {
                            ...
                            "untracked_private_field": "must be ignored",
                        },
                        ensure_ascii=False,
                    )
                    + "\n",
...
def test_human_score_entry_runner_pending_public_safe_contract(tmp_path: Path) -> None:
    audio_dir = tmp_path / "audio"
    _write_fixture_audio_files(audio_dir)

    report = entry.run_voice_local_tts_human_score_entry(
        private_audio_dir=audio_dir,
        private_score_entry_guide_path=tmp_path / "score_entry_guide.md",
        private_score_entry_draft_path=tmp_path / "scores.entry.template.jsonl",
        private_score_input_path=tmp_path / "missing_scores.jsonl",
        doc_path=tmp_path / "VOICE_LOCAL_TTS_HUMAN_SCORE_ENTRY.md",
        report_path=tmp_path / "voice_local_tts_human_score_entry_report.md",
        result_rows_path=tmp_path / "voice_local_tts_human_score_entry_rows.jsonl",
    )

    assert entry.collect_entry_failures(report) == []
    ...
    assert report.summary.private_score_entry_guide_created_count == 1
    assert report.summary.private_score_entry_draft_created_count == 1
    assert report.summary.private_score_entry_draft_row_count == 30
    ...
    assert report.summary.score_entry_decision == "pending_manual_score_entry"

    draft_rows = [
        json.loads(line)
        for line in (tmp_path / "scores.entry.template.jsonl")
    ...
    assert all("audio_file_name" in row for row in draft_rows)
    assert all("audio_path" not in row for row in draft_rows)

    guide = (tmp_path / "score_entry_guide.md").read_text(encoding="utf-8")
    assert "Voice Local TTS Human Score Entry Guide" in guide
    assert "경복궁은 조선의 중심 궁궐" in guide

    public_rows = [
        json.loads(line)
        for line in (tmp_path / "voice_local_tts_human_score_entry_rows.jsonl")
        .read_text(encoding="utf-8")
        .splitlines()
    ]
...
def test_human_score_entry_runner_aggregates_completed_private_scores(
    tmp_path: Path,
) -> None:
    audio_dir = tmp_path / "audio"
    ...
    assert (
        report.summary.score_entry_decision
...
def test_human_score_entry_docs_record_pending_state() -> None:
    ...
    assert entry.WORK_ID in doc
    assert entry.WORK_ID in report
...
def test_human_score_entry_keeps_forbidden_claims_forbidden() -> None:
    doc = DOC_PATH.read_text(encoding="utf-8")
