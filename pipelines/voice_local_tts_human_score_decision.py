...
DEFAULT_RESULT_ROWS_PATH = (
    ...
    / "evals"
    / "results"
    / "voice_local_tts_human_score_decision_public_rows.jsonl"
)
DEFAULT_DOC_PATH = Path("docs") / "VOICE_LOCAL_TTS_HUMAN_SCORE_DECISION.md"
DEFAULT_REPORT_PATH = (
    Path("evals") / "reports" / "voice_local_tts_human_score_decision_report.md"
)
ACCEPT_OVERALL_SCORE_AVG_THRESHOLD = 4.0
ACCEPT_MIN_CRITERION_SCORE_AVG_THRESHOLD = 3.5
REJECT_OVERALL_SCORE_AVG_THRESHOLD = 3.0
REJECT_MIN_CRITERION_SCORE_AVG_THRESHOLD = 2.5

TtsHumanScoreDecision = Literal[
    ...
    "candidate_rejected_by_human_scores",
    "candidate_pending_more_review",
    "candidate_accepted_for_demo_review",
]


class TtsHumanScoreDecisionBase(BaseModel):
    model_config = ConfigDict(extra="forbid", frozen=True)


class TtsHumanScoreDecisionSummary(TtsHumanScoreDecisionBase):
    ...
    provider_decision_public_row_count: int = Field(ge=0)
    overall_score_avg: float | None = None
    overall_score_min: int | None = None
    overall_score_max: int | None = None
    accept_overall_score_avg_threshold: float = ACCEPT_OVERALL_SCORE_AVG_THRESHOLD
    accept_min_criterion_score_avg_threshold: float = (
        ACCEPT_MIN_CRITERION_SCORE_AVG_THRESHOLD
    )
    reject_overall_score_avg_threshold: float = REJECT_OVERALL_SCORE_AVG_THRESHOLD
    reject_min_criterion_score_avg_threshold: float = (
        REJECT_MIN_CRITERION_SCORE_AVG_THRESHOLD
    )
    criterion_below_accept_threshold_count: int = Field(ge=0)
    criterion_below_reject_threshold_count: int = Field(ge=0)
    ...
    provider_decision: TtsHumanScoreDecision


class TtsHumanScoreDecisionReport(TtsHumanScoreDecisionBase):
    report_version: str = REPORT_VERSION
    score_decision_id: str = Field(min_length=1)
    ...
    summary: TtsHumanScoreDecisionSummary
    aggregates: tuple[TtsHumanCriterionAggregate, ...]
    output_quality: PublicRetrievalArtifactQuality
    qualitative_assessment: dict[str, str]


def run_voice_local_tts_human_score_decision(
    *,
    ...
    score_decision_id = build_score_decision_id(
        audio_rows=audio_rows,
        score_rows=score_rows,
        summary=summary,
    )
    public_rows = build_public_rows(
        score_decision_id=score_decision_id,
        summary=summary,
        aggregates=aggregates,
    )
    provisional_quality = measure_public_retrieval_artifact_quality(
        report_version=REPORT_VERSION,
        run_id=score_decision_id,
    ...
    summary = summary.model_copy(
        update={
            ...
            "provider_decision": build_decision(
                summary=summary,
    ...
    if failures:
        raise ValueError(f"voice local TTS human score decision gate failed: {failures}")
    ...
    print(
        "voice_local_tts_human_score_decision "
        f"status={report.summary.provider_decision} "
        f"completed_scores={report.summary.completed_score_row_count} "
        f"pending_scores={report.summary.pending_score_row_count} "
        f"overall_score_avg={format_optional_float(report.summary.overall_score_avg)} "
...
def build_summary(
    *,
    ...
    summary = TtsHumanScoreDecisionSummary(
        ...
        criterion_below_accept_threshold_count=sum(
            1
            for score_avg in criterion_avgs
            if score_avg < ACCEPT_MIN_CRITERION_SCORE_AVG_THRESHOLD
        ),
        criterion_below_reject_threshold_count=sum(
            1
            for score_avg in criterion_avgs
            if score_avg < REJECT_MIN_CRITERION_SCORE_AVG_THRESHOLD
        ),
...
def build_decision(
    *,
    ...
    if (
        summary.overall_score_avg is not None
        and summary.overall_score_avg < REJECT_OVERALL_SCORE_AVG_THRESHOLD
    ) or summary.criterion_below_reject_threshold_count:
        return "candidate_rejected_by_human_scores"
    if (
        summary.overall_score_avg is not None
        and summary.overall_score_avg >= ACCEPT_OVERALL_SCORE_AVG_THRESHOLD
        and summary.criterion_below_accept_threshold_count == 0
    ):
        return "candidate_accepted_for_demo_review"
    return "candidate_pending_more_review"


def collect_decision_failures(report: TtsHumanScoreDecisionReport) -> list[str]:
    ...
    if summary.provider_decision == "failed_public_safety_gate":
        failures.append("public_safety_gate_failed")
    return list(dict.fromkeys(failures))


def collect_decision_blockers(report: TtsHumanScoreDecisionReport) -> list[str]:
    summary = report.summary
    blockers: list[str] = []
    if summary.provider_decision == "blocked_missing_human_scores":
        blockers.append("missing_human_score_input")
    if summary.provider_decision == "blocked_incomplete_human_scores":
        blockers.append("incomplete_human_score_rows")
    if summary.provider_decision == "blocked_missing_private_audio":
        blockers.append("missing_private_audio")
    if summary.provider_decision == "blocked_invalid_private_scores":
        blockers.append("invalid_private_score_rows")
    if summary.provider_decision == "failed_public_safety_gate":
        blockers.append("public_safety_gate_failed")
    if summary.provider_decision == "candidate_rejected_by_human_scores":
        blockers.append("human_score_quality_below_reject_threshold")
    if summary.provider_decision == "candidate_pending_more_review":
        blockers.append("human_score_quality_pending_more_review")
    return list(dict.fromkeys(blockers))


def build_report(
    *,
    score_decision_id: str,
    scripts_path: Path,
    ...
    report = TtsHumanScoreDecisionReport(
        score_decision_id=score_decision_id,
...
def build_public_rows(
    *,
    ...
    rows: list[dict[str, Any]] = [
        {
            "row_type": "local_tts_human_score_provider_decision_summary",
            "score_decision_id": score_decision_id,
            "provider_candidate_id": PROVIDER_CANDIDATE_ID,
            "model_family": MODEL_FAMILY,
            "provider_decision": summary.provider_decision,
            ...
            "accept_overall_score_avg_threshold": summary.accept_overall_score_avg_threshold,
            "accept_min_criterion_score_avg_threshold": (
                summary.accept_min_criterion_score_avg_threshold
            ),
            "criterion_below_accept_threshold_count": (
                summary.criterion_below_accept_threshold_count
            ),
    ...
    rows.extend(
        {
            "row_type": "local_tts_human_score_provider_decision_aggregate",
            "score_decision_id": score_decision_id,
            "provider_candidate_id": PROVIDER_CANDIDATE_ID,
            "model_family": MODEL_FAMILY,
...
| provider_decision_public_row_count | {summary.provider_decision_public_row_count} |
| overall_score_avg | {format_optional_float(summary.overall_score_avg)} |
| criterion_below_accept_threshold_count | {summary.criterion_below_accept_threshold_count} |
| criterion_below_reject_threshold_count | {summary.criterion_below_reject_threshold_count} |
| external_provider_call_count | {summary.external_provider_call_count} |
| external_audio_transmission_count | {summary.external_audio_transmission_count} |
...
| decision | condition |
| --- | --- |
| `candidate_accepted_for_demo_review` | 30개 score 완료, overall 평균 {ACCEPT_OVERALL_SCORE_AVG_THRESHOLD:.1f} 이상, 모든 criterion 평균 {ACCEPT_MIN_CRITERION_SCORE_AVG_THRESHOLD:.1f} 이상 |
| `candidate_pending_more_review` | 30개 score 완료지만 채택/탈락 threshold 사이 |
| `candidate_rejected_by_human_scores` | overall 평균 {REJECT_OVERALL_SCORE_AVG_THRESHOLD:.1f} 미만 또는 criterion 평균 {REJECT_MIN_CRITERION_SCORE_AVG_THRESHOLD:.1f} 미만 존재 |
...
def build_markdown(report: TtsHumanScoreDecisionReport) -> str:
    summary = report.summary
...
| score_decision_id | `{report.score_decision_id}` |
...
| provider_decision_status | `{summary.provider_decision}` |
...
| accept_overall_score_avg_threshold | {summary.accept_overall_score_avg_threshold:.6f} |
| accept_min_criterion_score_avg_threshold | {summary.accept_min_criterion_score_avg_threshold:.6f} |
| reject_overall_score_avg_threshold | {summary.reject_overall_score_avg_threshold:.6f} |
| reject_min_criterion_score_avg_threshold | {summary.reject_min_criterion_score_avg_threshold:.6f} |
| criterion_below_accept_threshold_count | {summary.criterion_below_accept_threshold_count} |
| criterion_below_reject_threshold_count | {summary.criterion_below_reject_threshold_count} |
...
```text
tts_human_score_decision_failures={failures}
tts_human_score_decision_blockers={blockers}
```
...
def build_qualitative(report: TtsHumanScoreDecisionReport) -> dict[str, str]:
    summary = report.summary
    return {
        ...
        "decision": summary.provider_decision,
    }
...
def build_score_decision_id(
    *,
    ...
    return f"voice-local-tts-human-score-decision-s{summary.selected_script_count}-{digest}"
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    run_voice_local_tts_human_score_decision(
