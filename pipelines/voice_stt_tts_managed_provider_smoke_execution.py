...
DEFAULT_DOC_PATH = Path("docs") / "VOICE_STT_TTS_MANAGED_PROVIDER_SMOKE_EXECUTION.md"
DEFAULT_REPORT_PATH = (
    Path("evals")
    / "reports"
    / "voice_stt_tts_managed_provider_smoke_execution_report.md"
)
DEFAULT_RESULT_ROWS_PATH = (
    Path("private_data")
    / "evals"
    / "results"
    / "voice_stt_tts_managed_provider_smoke_execution_rows.jsonl"
)
EXPLICIT_AZURE_ACTUAL_CALL_APPROVAL_PHRASE = (
    "승인, Azure 실제 호출 포함해서 작업 진행해"
)

ManagedSmokeExecutionDecision = Literal[
    "blocked_missing_azure_credentials",
    "blocked_source_recheck_incomplete",
    "blocked_missing_user_external_call_approval",
    "blocked_missing_explicit_actual_call_phrase",
    "ready_for_actual_azure_smoke_execution",
    "blocked_by_public_safety_gate",
]
ManagedSmokeScriptStatus = Literal["blocked_not_executed", "ready_not_executed"]


class ManagedSmokeExecutionModel(BaseModel):
    model_config = ConfigDict(extra="forbid", frozen=True)


class ManagedSmokeExecutionSourceRow(ManagedSmokeExecutionModel):
    source_check_type: str = Field(min_length=1)
    recheck_required_before_execution: bool
    recheck_completed_for_execution: bool


class ManagedSmokeExecutionScriptRow(ManagedSmokeExecutionModel):
    ...
    row_status: ManagedSmokeScriptStatus


class ManagedSmokeExecutionSummary(ManagedSmokeExecutionModel):
    ...
    explicit_actual_call_approval_phrase_valid: bool
    managed_provider_execution_requested_count: int = Field(ge=0)
    managed_provider_execution_ready: bool
    managed_provider_execution_approved: bool
    managed_provider_api_call_count: int = Field(ge=0)
    external_audio_transmission_count: int = Field(ge=0)
    ...
    execution_decision: ManagedSmokeExecutionDecision


class ManagedProviderSmokeExecutionReport(ManagedSmokeExecutionModel):
    ...
    source_rechecks: tuple[ManagedSmokeExecutionSourceRow, ...]
    script_plan_rows: tuple[ManagedSmokeExecutionScriptRow, ...]
    metric_plan_rows: tuple[AzureSmokeMetricPlanRow, ...]
    summary: ManagedSmokeExecutionSummary
    output_quality: PublicRetrievalArtifactQuality
    qualitative_assessment: dict[str, str]


def run_voice_stt_tts_managed_provider_smoke_execution(
    *,
    ...
    external_call_approval_phrase: str = "",
) -> ManagedProviderSmokeExecutionReport:
    ...
    summary = build_summary(
        ...
        external_call_approval_phrase=external_call_approval_phrase,
    )
    if execute_managed_provider and not summary.managed_provider_execution_approved:
        raise ValueError(
            "managed provider smoke execution blocked by gate; credentials, "
            "source recheck, region, retention, cost, user approval, and the "
            "explicit Azure actual-call approval phrase are required.",
        )

    script_rows = build_script_plan_rows(
        scripts=scripts,
        ready_not_executed=summary.managed_provider_execution_ready,
    )
    ...
    failures = collect_managed_provider_smoke_execution_failures(report)
    if failures:
        raise ValueError(f"managed provider smoke execution gate failed: {failures}")
    ...
    print(
        ...
        f"managed_provider_execution_approved="
        f"{str(report.summary.managed_provider_execution_approved).lower()} "
...
def build_source_rechecks(
    *,
    completed: bool,
) -> tuple[ManagedSmokeExecutionSourceRow, ...]:
    return tuple(
        ManagedSmokeExecutionSourceRow(
...
) -> tuple[ManagedSmokeExecutionScriptRow, ...]:
    row_status: ManagedSmokeScriptStatus = (
        "ready_not_executed" if ready_not_executed else "blocked_not_executed"
    )
    return tuple(
        ManagedSmokeExecutionScriptRow(
...
def build_summary(
    *,
    ...
    explicit_phrase_valid = (
        external_call_approval_phrase == EXPLICIT_AZURE_ACTUAL_CALL_APPROVAL_PHRASE
    )
    execution_ready = (
    ...
    execution_requested_count = 1 if execute_managed_provider else 0
    execution_approved = (
        execution_ready and execute_managed_provider and explicit_phrase_valid
    )
    summary = ManagedSmokeExecutionSummary(
        provider_candidate_count=1,
        selected_provider_candidate_id=PROVIDER_CANDIDATE_ID,
        ...
        cost_confirmation_completed=cost_confirmation_completed,
        user_external_call_approval_recorded=user_external_call_approval_recorded,
        explicit_actual_call_approval_phrase_valid=explicit_phrase_valid,
        managed_provider_execution_requested_count=execution_requested_count,
        managed_provider_execution_ready=execution_ready,
        managed_provider_execution_approved=execution_approved,
        managed_provider_api_call_count=0,
        external_audio_transmission_count=0,
...
def build_execution_decision(
    *,
    ...
    if not summary.explicit_actual_call_approval_phrase_valid:
        return "blocked_missing_explicit_actual_call_phrase"
...
def build_report(
    *,
    ...
    source_rechecks: tuple[ManagedSmokeExecutionSourceRow, ...],
    script_rows: tuple[ManagedSmokeExecutionScriptRow, ...],
    metric_rows: tuple[AzureSmokeMetricPlanRow, ...],
    summary: ManagedSmokeExecutionSummary,
    output_quality: PublicRetrievalArtifactQuality,
) -> ManagedProviderSmokeExecutionReport:
    report = ManagedProviderSmokeExecutionReport(
        execution_id=execution_id,
    ...
    rows: list[dict[str, Any]] = [
        {
            ...
            "managed_provider_execution_ready": (
                summary.managed_provider_execution_ready
            ),
            "managed_provider_execution_approved": (
                summary.managed_provider_execution_approved
            ),
...
def collect_managed_provider_smoke_execution_failures(
    report: ManagedProviderSmokeExecutionReport,
) -> list[str]:
...
def build_doc_markdown(report: ManagedProviderSmokeExecutionReport) -> str:
    summary = report.summary
    source_rows = "\n".join(_format_source_row(row) for row in report.source_rechecks)
    script_rows = "\n".join(_format_script_row(row) for row in report.script_plan_rows)
    metric_rows = "\n".join(_format_metric_row(row) for row in report.metric_plan_rows)
    return f"""# Voice STT/TTS Managed Provider Smoke Execution

## 결론

`{WORK_ID}`는 selected managed provider smoke execution work order를 만든다.

현재 `execution_decision={summary.execution_decision}`이며, managed provider API call과 external audio transmission은 0회다.
...
| `managed_provider_execution_ready` | `{str(summary.managed_provider_execution_ready).lower()}` |
| `managed_provider_execution_approved` | `{str(summary.managed_provider_execution_approved).lower()}` |
| `execution_decision` | `{summary.execution_decision}` |
| `managed_provider_api_call_count` | `{summary.managed_provider_api_call_count}` |
...
def build_report_markdown(report: ManagedProviderSmokeExecutionReport) -> str:
    summary = report.summary
    ...
    failures = collect_managed_provider_smoke_execution_failures(report)
    return f"""# Voice STT/TTS Managed Provider Smoke Execution Report
...
| explicit_actual_call_approval_phrase_valid | {str(summary.explicit_actual_call_approval_phrase_valid).lower()} |
...
def build_qualitative_assessment(
    ...
    elif summary.execution_decision == "blocked_missing_explicit_actual_call_phrase":
        operations_boundary = "actual-call 승인 문구가 없어 실행하지 않는다."
    else:
...
def build_execution_id(
    *,
    ...
    return "managed-provider-smoke-execution-" + _stable_digest(
        {
...
def _format_source_row(row: ManagedSmokeExecutionSourceRow) -> str:
    return (
...
def _format_script_row(row: ManagedSmokeExecutionScriptRow) -> str:
    return (
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Build managed provider STT/TTS smoke execution gate.",
    )
    ...
    parser.add_argument("--execute-managed-provider", action="store_true")
    parser.add_argument("--external-call-approval-phrase", default="")
    return parser.parse_args()


def main() -> int:
    args = parse_args()
    run_voice_stt_tts_managed_provider_smoke_execution(
        env_path=args.env,
        ...
        external_call_approval_phrase=args.external_call_approval_phrase,
    )
    return 0
