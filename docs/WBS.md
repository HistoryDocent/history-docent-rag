...
| 9.37 | voice local sherpa-onnx Supertonic 3 Korean TTS smoke | sherpa-onnx Korean TTS runner, docs/report/test | sherpa-onnx runtime 설치, Supertonic 3 Korean private model 확인, public-safe script 5개 private wav 합성, external provider call 0 기록 | `평가: sherpa-onnx 한국어 TTS smoke 추가` |
| 9.38 | voice local TTS quality listening review | local TTS quality metric runner, docs/report/test | sherpa-onnx private wav 5개 자동 sanity metric과 human listening rubric 생성, human completed 0, external provider call 0 기록 | `평가: 로컬 TTS 청취 품질 평가 기준 추가` |
| 9.39 | voice local TTS human score fill | private human score template, public aggregate runner, docs/report/test | 5개 script x 6 rubric private score template 생성, score 미입력 상태는 pending, public aggregate row 6, external provider call 0 기록 | `평가: 로컬 TTS 사람 청취 점수 기준 추가` |
| 9.40 | voice local TTS human score collection | private listening manifest, private guide, public aggregate runner, docs/report/test | private wav 5개 청취 평가 manifest와 guide 생성, score 미입력 상태는 collection-ready, public aggregate row 6, external provider call 0 기록 | `평가: 로컬 TTS 사람 청취 점수 수집 절차 추가` |
| 9.41 | voice local TTS human score entry | private score entry guide, private draft, public aggregate runner, docs/report/test | private entry guide와 draft 30행 생성, score 미입력 상태는 pending_manual_score_entry, public aggregate row 6, external provider call 0 기록 | `평가: 로컬 TTS 사람 청취 점수 입력 도구 추가` |
| 9.42 | voice local TTS human score entry completion | private score input completion verifier, public aggregate runner, docs/report/test | private score input이 없으면 blocked_missing_human_scores, 완료 fixture는 score 30행과 aggregate row 6 검증, external provider call 0 기록 | `평가: 로컬 TTS 사람 청취 점수 입력 완료 검증 추가` |
| 9.43 | voice local TTS human score manual scoring workspace | private HTML score sheet, private draft, public aggregate runner, docs/report/test | private HTML score sheet 생성, score 미입력 상태는 ready_for_human_manual_scoring, 완료 fixture는 score 30행 검증, external provider call 0 기록 | `평가: 로컬 TTS 사람 청취 수동 채점 workspace 추가` |
| 9.44 | voice local whisper.cpp deployment smoke | whisper.cpp runtime/model preflight, docs/report/test | 현재 환경에서 `whisper-cli` runtime과 `ggml` model file 부재를 blocker로 기록하고, CUDA preflight와 external provider call 0을 검증 | `평가: whisper.cpp 로컬 STT 배포성 smoke 추가` |
| 9.45 | voice local TTS human score provider decision | human score decision runner, docs/report/test | private score 30행 미완료 시 TTS provider 채택을 차단하고, 완료 fixture는 채택/탈락 threshold를 검증, external provider call 0 기록 | `평가: 로컬 TTS 사람 청취 점수 기반 후보 결정 추가` |
...
| 9.50 | voice demo stack decision | `docs/VOICE_DEMO_STACK_DECISION.md`, report, test | `HD-VOICE-DEMO-STACK-DECISION-001`에서 사람 청취 점수 30/30 평균 5.0을 반영해 `faster-whisper small CUDA` STT와 `sherpa-onnx Supertonic 3 Korean` TTS demo 후보를 정리하고, TTS final provider 0과 external provider call 0을 검증 | `docs: 로컬 음성 데모 스택 판단 정리` |
| 9.51 | voice demo playback smoke | playback smoke runner, docs/report/test | `HD-VOICE-DEMO-PLAYBACK-SMOKE-001`에서 private wav 5개 playback-ready, 사람 청취 30/30, speaker 자동 재생 0, external provider call 0을 검증 | `test: 로컬 음성 데모 playback smoke 추가` |
| 9.52 | voice API local runtime route smoke | route smoke runner, docs/report/test | `HD-VOICE-API-LOCAL-RUNTIME-ROUTE-SMOKE-001`에서 local voice route 기본 비활성화, 명시 flag contract response, validation reject, external provider call 0을 검증 | `test: 로컬 음성 API route smoke 추가` |
...
| 9.57 | portfolio walkthrough script | `docs/PORTFOLIO_WALKTHROUGH_SCRIPT.md`, report, test | `HD-PORTFOLIO-WALKTHROUGH-SCRIPT-001`에서 3분 설명 script와 demo click path를 고정하고 production claim을 추가하지 않음 | `docs: 포트폴리오 walkthrough 스크립트 추가` |
...
| 9.62 | voice local whisper.cpp install approval | `docs/VOICE_LOCAL_WHISPERCPP_INSTALL_APPROVAL.md`, report, test | `HD-VOICE-LOCAL-WHISPERCPP-INSTALL-APPROVAL-001`에서 build/download 실행 전 승인 기준과 public artifact 경계를 고정 | `docs: whisper.cpp 설치 승인 gate 추가` |
| 9.63 | voice local whisper.cpp install readiness | `docs/VOICE_LOCAL_WHISPERCPP_INSTALL_READINESS.md`, report, test | `HD-VOICE-LOCAL-WHISPERCPP-INSTALL-READINESS-001`에서 toolchain/GPU/runtime/model 상태를 읽기 전용으로 점검하고 source build blocker를 기록 | `docs: whisper.cpp 설치 readiness 추가` |
| 9.64 | voice local whisper.cpp install strategy | `docs/VOICE_LOCAL_WHISPERCPP_INSTALL_STRATEGY.md`, report, test | `HD-VOICE-LOCAL-WHISPERCPP-INSTALL-STRATEGY-001`에서 source build/prebuilt/keep faster-whisper 3개 선택지를 비교하고 설치 보류 전략을 채택 | `docs: whisper.cpp 설치 전략 결정 추가` |
| 9.65 | GitHub push readiness v2 | `docs/GITHUB_PUSH_READINESS_V2.md`, report, test | `HD-GITHUB-PUSH-READINESS-V2-001`에서 최신 local commit 이후 remote/branch/worktree/commit scope/tracked artifact를 재점검하고 실제 push는 실행하지 않음 | `docs: GitHub push readiness v2 추가` |
