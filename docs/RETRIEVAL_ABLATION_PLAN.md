...
Solar Pro 3 provider contract 실행 결과 v1:

공식 Upstage Chat Completions API 형식에 맞춰 Solar Pro 3 provider를 구현했다. provider는 `CitationRagDraft`만 생성하고, citation 결합은 기존 `CitationRagAnswerAssembler`가 담당한다.
