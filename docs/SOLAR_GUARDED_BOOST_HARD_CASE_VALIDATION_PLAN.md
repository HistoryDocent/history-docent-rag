...
guarded boost hard-case validation runner를 구현하고 실행했다.

실행 조건:

- private `place_story` dev 10개
- 기존 HD-SOLAR-016 public-safe live paired metric row 사용
- 현재 input-only route decision 재계산
- Solar Pro 3 추가 호출 0회
...
- 현재 guardrail은 기존 live route decision과 불일치가 없다.
- candidate 자동 선택 1건은 live safety metric을 악화시키지 않았다.
- manual review 2건은 자동 선택하지 않는 현재 정책을 유지한다.
- doc guardrail 차단도 유지한다.
