...
ColBERT-style late interaction은 dev hard subset에서 top50 MRR만 소폭 상승했지만 nDCG@5와 latency가 악화되어 기본 route로 채택하지 않는다. 이후 voice UI MVP 계획과 frontend skeleton을 완료했다. 다만 STT/TTS 검증, live Solar Pro 3 호출, frontend/backend contract smoke는 아직 수행하지 않았다.
...
| ---: | --- | --- | --- | --- |
| 1 | `HD-VOICE-UI-003` | frontend/backend contract smoke | skeleton이 fixture-only라 실제 FastAPI contract-only response 연결을 별도 검증해야 한다. | 예 |
