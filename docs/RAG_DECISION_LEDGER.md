...
| `portfolio_summary` | `HD-PORTFOLIO-001` | public README/docs summary | summarized_stage_count=26, leakage_count=0 | implemented | public-safe-summary | `evals/reports/portfolio_result_summary_report.md` |
...
| ---: | --- | --- | --- | --- |
| 1 | `HD-API-ROUTER-005` | API active route flag dry-run contract | relationship route를 바로 켜지 않고 default disabled flag로 관찰해야 한다. | 예 |
| 2 | `HD-LOCKED-RETRIEVAL-001` | locked test 최종 retrieval candidate 검증 계획 | API dry-run contract 이후 locked split 사용 기준이 필요하다. | 예 |
