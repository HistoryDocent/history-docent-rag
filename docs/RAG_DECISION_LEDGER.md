...
| voice TTS | runtime gate 통과 전 보류 | CUDA는 감지됐지만 MeloTTS runtime 미설치로 실제 합성은 차단, external provider call 0 |
| voice local runtime | runtime matrix 완료 | 후보 5개 중 import 가능 후보는 `openai-whisper` fallback 1개, TTS runtime 가능 후보는 0개 |
...
| ---: | --- | --- | --- | --- |
| 1 | `HD-VOICE-STT-TTS-LOCAL-TTS-RUNTIME-INSTALL-001` | local TTS runtime candidate install/retry from matrix | 현재 TTS runtime 가능 후보가 0개다. MeloTTS 우선, 실패 시 sherpa-onnx 대체 후보 순서로 실제 private wav 생성 가능성을 확인해야 한다. | 예 |
| 2 | `HD-VOICE-STT-TTS-LOCAL-MELOTTS-RUNTIME-RETRY-001` | local MeloTTS runtime install and retry smoke execution | MeloTTS는 1순위 TTS 후보지만 현재 Python/Windows 환경에서 설치 호환성 리스크가 있다. | 예 |
...
- voice local runtime matrix에서 무료 로컬 후보 5개의 import/runtime/CUDA preflight를 기록했고, 현재 TTS runtime 가능 후보는 0개임을 확인했다.
...
README 결과 표와 포트폴리오 메시지 정리는 완료했다. query type classifier baseline, 오분류 failure analysis, `/chat` dry-run field 연결, relationship guard 평가, guarded route 후보 dry-run 노출, failure analysis 10개 정리, `place_story` targeted chunk audit, HyDE subset readiness, HyDE live paired retrieval comparison, HyDE larger dev subset readiness, HyDE larger live paired retrieval comparison, active routing 적용 판단 계획, active route shadow evaluation, API active route flag dry-run contract, locked retrieval 검증 승인 계획, locked retrieval readiness dry-run runner, locked retrieval execution approval, locked retrieval paired comparison, final ablation report, API response sample, portfolio QA, submission ready audit, ColBERT-style plan-only gate, execution approval gate, hard subset 비교, voice UI MVP plan, voice UI skeleton, voice UI contract smoke, real browser voice UI visual QA, portfolio demo runbook, public repository audit refresh, portfolio submission rehearsal, voice STT/TTS planning, voice STT/TTS contract skeleton, voice STT/TTS provider benchmark plan, voice STT/TTS provider benchmark readiness, voice STT/TTS provider benchmark execution approval, voice STT/TTS provider benchmark local smoke, voice STT/TTS local model ablation, voice STT/TTS managed provider smoke approval, voice STT/TTS managed provider smoke execution harness, voice STT/TTS managed provider smoke preflight, voice STT/TTS managed provider smoke execution gate, voice STT/TTS Azure managed smoke readiness, voice STT/TTS Azure credential preflight, voice STT/TTS Azure smoke execution approval, voice STT/TTS Azure smoke execution gate, voice STT/TTS Azure credential ready smoke approval, voice provider local-first decision, voice STT/TTS local TTS smoke gate, voice local runtime matrix까지 완료했다. 필수 포트폴리오 제출 gate는 여기서 완료이며, 다음에는 managed credential 준비가 아니라 local TTS runtime 후보를 분리 설치하고 smoke를 재실행한다. managed provider live smoke는 optional paid comparison으로만 남긴다.
