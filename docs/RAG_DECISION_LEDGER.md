...
ColBERT-style late interaction은 dev hard subset에서 top50 MRR만 소폭 상승했지만 nDCG@5와 latency가 악화되어 기본 route로 채택하지 않는다. 이후 voice UI MVP 계획, frontend skeleton, frontend/backend contract smoke, real browser visual QA, portfolio demo runbook, public repository audit refresh, portfolio submission rehearsal, voice STT/TTS planning gate, provider 호출 없는 voice STT/TTS contract skeleton, provider benchmark plan, provider benchmark readiness, provider benchmark execution approval, local smoke, local model ablation, managed provider smoke approval, managed provider smoke execution harness를 완료했다. 다만 managed provider 실제 STT/TTS 품질 검증, live Solar Pro 3 voice demo, production 배포는 아직 수행하지 않았다.
...
| ---: | --- | --- | --- | --- |
| 1 | `HD-VOICE-STT-TTS-PROVIDER-BENCH-MANAGED-SMOKE-001` | optional voice STT/TTS managed provider smoke execution | managed provider smoke approval은 완료됐다. 다음 단계는 실제 외부 audio 전송과 비용 발생 가능성이 있으므로 provider별 call cap, region, retention 재확인 후 별도 승인으로만 실행한다. | 예 |
