...
ColBERT-style late interaction은 dev hard subset에서 top50 MRR만 소폭 상승했지만 nDCG@5와 latency가 악화되어 기본 route로 채택하지 않는다. 이후 voice UI MVP 계획, frontend skeleton, frontend/backend contract smoke, real browser visual QA, portfolio demo runbook, public repository audit refresh, portfolio submission rehearsal, voice STT/TTS planning gate, provider 호출 없는 voice STT/TTS contract skeleton, provider benchmark plan, provider benchmark readiness, provider benchmark execution approval, local smoke, local model ablation, managed provider smoke approval, managed provider smoke execution harness, managed provider smoke preflight, Azure managed smoke readiness, Azure credential preflight, Azure smoke execution approval을 완료했다. 다만 managed provider 실제 STT/TTS 품질 검증, live Solar Pro 3 voice demo, production 배포는 아직 수행하지 않았다.
...
| ---: | --- | --- | --- | --- |
| 1 | `HD-VOICE-STT-TTS-PROVIDER-BENCH-MANAGED-SMOKE-001` | optional voice STT/TTS managed provider smoke execution | Azure smoke execution approval 결과 현재 credential missing과 source recheck 미완료로 실행 승인은 false다. credential 준비 후 preflight와 approval gate를 다시 통과한 뒤 별도 승인으로만 실행한다. | 예 |
...
README 결과 표와 포트폴리오 메시지 정리는 완료했다. query type classifier baseline, 오분류 failure analysis, `/chat` dry-run field 연결, relationship guard 평가, guarded route 후보 dry-run 노출, failure analysis 10개 정리, `place_story` targeted chunk audit, HyDE subset readiness, HyDE live paired retrieval comparison, HyDE larger dev subset readiness, HyDE larger live paired retrieval comparison, active routing 적용 판단 계획, active route shadow evaluation, API active route flag dry-run contract, locked retrieval 검증 승인 계획, locked retrieval readiness dry-run runner, locked retrieval execution approval, locked retrieval paired comparison, final ablation report, API response sample, portfolio QA, submission ready audit, ColBERT-style plan-only gate, execution approval gate, hard subset 비교, voice UI MVP plan, voice UI skeleton, voice UI contract smoke, real browser voice UI visual QA, portfolio demo runbook, public repository audit refresh, portfolio submission rehearsal, voice STT/TTS planning, voice STT/TTS contract skeleton, voice STT/TTS provider benchmark plan, voice STT/TTS provider benchmark readiness, voice STT/TTS provider benchmark execution approval, voice STT/TTS provider benchmark local smoke, voice STT/TTS local model ablation, voice STT/TTS managed provider smoke approval, voice STT/TTS managed provider smoke execution harness, voice STT/TTS managed provider smoke preflight, voice STT/TTS Azure managed smoke readiness, voice STT/TTS Azure credential preflight, voice STT/TTS Azure smoke execution approval까지 완료했다. 필수 포트폴리오 제출 gate는 여기서 완료이며, 다음에는 credential 준비와 별도 승인 후 optional voice STT/TTS managed provider smoke execution으로 넘어갈 수 있다.
