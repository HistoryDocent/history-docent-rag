...
| ---: | --- | --- | --- | --- |
| 1 | `HD-VOICE-STT-TTS-PROVIDER-BENCH-READINESS-001` | optional voice STT/TTS provider benchmark readiness | provider benchmark plan은 완료됐다. 실제 provider 호출 전 public-safe fixture, provider config skeleton, CUDA runtime preflight, pricing/privacy source recheck field를 no-live-call로 검증해야 한다. | 예 |
