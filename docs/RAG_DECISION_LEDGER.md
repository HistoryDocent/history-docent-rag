...
그 다음 local free voice stack lock에서 `faster-whisper small CUDA`는 현재 demo evidence 기준 primary STT 후보로 고정했고, TTS는 final provider 0 상태로 차단했다. 이후 whisper.cpp deployment retry에서 현재 환경을 재점검했지만 runtime/model 부재가 유지되어 `whisper.cpp` 실행 성공이나 provider 채택은 주장하지 않는다.
...
| 2 | `HD-VOICE-LOCAL-WHISPERCPP-INSTALL-APPROVAL-001` | optional whisper.cpp runtime/model install approval | 현재 재점검에서도 runtime/model이 없어 `whisper.cpp` 실행이 0건이다. build/download는 로컬 환경 변경이므로 별도 승인 기준이 필요하다. | 예 |
