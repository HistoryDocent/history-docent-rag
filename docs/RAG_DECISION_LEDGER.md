...
그 다음 local free voice stack lock에서 `faster-whisper small CUDA`는 현재 demo evidence 기준 primary STT 후보로 고정했고, TTS는 final provider 0 상태로 차단했다.
...
| 2 | `HD-VOICE-STT-TTS-LOCAL-MELOTTS-RUNTIME-FIX-001` | optional MeloTTS Windows dependency fix | MeloTTS는 model load까지 통과했지만 Korean synthesis가 `eunjeon` Windows build dependency로 차단됐다. MSVC 설치나 대체 G2P는 별도 승인 대상이다. | 예 |
| 3 | `HD-VOICE-STT-TTS-PROVIDER-BENCH-MANAGED-SMOKE-LIVE-001` | optional paid voice STT/TTS managed provider live smoke execution | Azure/Google/AWS는 기본 경로가 아니다. 비용, region, retention, credential, actual-call 승인 문구가 모두 충족될 때만 optional comparison으로 실행한다. | 예 |
