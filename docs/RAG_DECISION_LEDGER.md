...
ColBERT-style late interaction은 dev hard subset에서 top50 MRR만 소폭 상승했지만 nDCG@5와 latency가 악화되어 기본 route로 채택하지 않는다. 이후 voice UI MVP 계획, frontend skeleton, frontend/backend contract smoke, real browser visual QA, portfolio demo runbook, public repository audit refresh, portfolio submission rehearsal, voice STT/TTS planning gate, provider 호출 없는 voice STT/TTS contract skeleton, provider benchmark plan, provider benchmark readiness를 완료했다. 다만 STT/TTS 품질 검증, live Solar Pro 3 voice demo, production 배포는 아직 수행하지 않았다.
...
| ---: | --- | --- | --- | --- |
| 1 | `HD-VOICE-STT-TTS-PROVIDER-BENCH-EXECUTION-APPROVAL-001` | optional voice STT/TTS provider benchmark execution approval | readiness는 완료됐다. 실제 provider 호출 전 call cap, 비용, region, 개인정보 경계, audio artifact 보관 위치를 다시 승인해야 한다. | 예 |
