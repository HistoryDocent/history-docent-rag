...
현재 기준선은 `C0 current parent-child`로 고정한다. 실패 사례 10개도 전역 청킹 재실험 근거가 아니라 targeted audit 근거만 남겼다. 다음 개발은 청킹 재실험이 아니라 query type별 routing 정책과 HyDE 같은 제한 실험을 분리하는 방향이 맞다.
...
| `portfolio_summary` | `HD-PORTFOLIO-001` | public README/docs summary | summarized_stage_count=19, leakage_count=0 | implemented | public-safe-summary | `evals/reports/portfolio_result_summary_report.md` |
...
| ---: | --- | --- | --- | --- |
| 1 | `HD-HYDE-001` | HyDE overview/relationship/no-answer subset 비교 | retrieval miss와 no-answer risk가 남아 있지만 LLM 비용과 hallucination guard가 필요하다. | 예 |
| 2 | `HD-CHUNK-AUDIT-001` | `place_story` 1건 targeted chunk audit | 전역 재청킹이 아니라 child/parent grain 손실 여부만 확인한다. | 예 |
...
- 실패 사례 10개를 원문 없이 분류했고 전역 청킹 재실험 대신 targeted audit으로 경계를 좁혔다.
...
README 결과 표와 포트폴리오 메시지 정리는 완료했다. query type classifier baseline, 오분류 failure analysis, `/chat` dry-run field 연결, relationship guard 평가, guarded route 후보 dry-run 노출, failure analysis 10개 정리도 통과했다. 다음에는 active routing이 아니라 HyDE 비용 실험 또는 targeted chunk audit이 우선이다.
