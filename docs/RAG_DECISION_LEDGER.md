...
ColBERT-style late interaction은 dev hard subset에서 top50 MRR만 소폭 상승했지만 nDCG@5와 latency가 악화되어 기본 route로 채택하지 않는다. 이후 voice UI MVP 계획, frontend skeleton, frontend/backend contract smoke를 완료했다. 다만 STT/TTS 검증, live Solar Pro 3 호출, real browser visual QA는 아직 수행하지 않았다.
...
| ---: | --- | --- | --- | --- |
| 1 | `HD-VOICE-UI-004` | real browser voice UI visual QA | contract 연결은 검증했지만 실제 browser layout, drawer, fallback 상태의 시각 검증이 남아 있다. | 예 |
