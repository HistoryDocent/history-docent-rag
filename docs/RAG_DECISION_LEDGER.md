...
| ---: | --- | --- | --- | --- |
| 1 | `HD-ROUTER-001` | query-type router decision report | dense voice rewrite, relationship hybrid, place_story guarded route 후보가 이미 존재한다. 다음은 route policy 정리다. | 예 |
...
다만 다음 단계에서 새 기법을 더 붙이기 전에 `query-type router`를 정리해야 한다. 현재 결과는 단일 retrieval method 하나가 모든 query type에서 최선이라는 결론이 아니기 때문이다.
