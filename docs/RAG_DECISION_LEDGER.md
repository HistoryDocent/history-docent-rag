...
ColBERT-style late interaction은 dev hard subset에서 top50 MRR만 소폭 상승했지만 nDCG@5와 latency가 악화되어 기본 route로 채택하지 않는다. 이후 voice UI MVP는 구현 전 계획 gate로만 고정했고 frontend 구현, STT/TTS 검증, live Solar Pro 3 호출은 수행하지 않았다.
...
| ---: | --- | --- | --- | --- |
| 1 | `HD-VOICE-UI-002` | frontend/voice UI skeleton | voice UI MVP 계획과 `/api/v1/chat` mapping이 고정됐으므로 contract fixture 기반 화면을 구현한다. | 예 |
