...
핵심은 한국사 도서 parser 결과를 citation 가능한 RAG corpus로 정리하고, 청킹, retrieval, reranker, query rewrite, evidence packing, generation, GraphRAG-lite, RAPTOR-lite, query type router, API dry-run, route guard, guarded route API 관찰 필드, 실패 사례 10개, targeted chunk audit, HyDE readiness, HyDE live 비교, HyDE larger readiness, HyDE larger live 비교, active route shadow evaluation, active route flag dry-run contract, locked retrieval 검증 승인 계획, locked retrieval readiness, locked retrieval execution approval을 같은 평가 원칙으로 비교해 채택, 보류, 기각을 분리했다는 점이다.
...
| locked retrieval | 미실행 | validation plan, readiness, execution approval 통과, metric 실행은 별도 승인 필요 |
...
| priority | work_id | 이유 |
| ---: | --- | --- |
| 1 | `HD-LOCKED-RETRIEVAL-004` | locked retrieval paired comparison runner 실행 |
...
다만 “성능 개선”보다 “평가 기반 의사결정”을 강조해야 한다. HyDE도 40개 확대 live 비교에서 기본값으로 기각했고 active routing도 shadow 평가 후 바로 켜지 않았으며 locked retrieval도 execution approval까지만 통과했으므로, 최종 제출 문구에서는 좋은 수치보다 채택하지 않은 이유를 같이 써야 한다.
