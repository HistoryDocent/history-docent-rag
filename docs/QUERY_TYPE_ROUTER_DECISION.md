...
RAPTOR-lite는 아직 router 정책에 넣지 않는다. overview/place_story 장문 맥락 질문에서 별도 input-only 비교를 진행한 뒤 route 후보 여부를 판단한다.

## Router Implementation Boundary

이번 결정은 route policy 문서화다. 아직 다음을 주장하지 않는다.

- query type classifier 구현 완료
- production routing 완료
- locked test 성능 개선
- Solar Pro 3 답변 품질 개선
- 통계적으로 유의미한 최종 개선

다음 구현에서는 deterministic query type label을 입력으로 받는 router skeleton부터 만든다. classifier 자체는 별도 실험 대상으로 분리한다.
...
| --- | --- | --- | --- | --- | --- |
| HD-ROUTER-002 | HD-ROUTER-001 | deterministic query-type router skeleton 구현 | route table unit test, relationship route branch test, no-answer abstain branch test, public leakage 0 | Medium | router module revert |
| HD-RAPTOR-001 | HD-ROUTER-001 | RAPTOR-lite overview/place_story input-only 비교 | same eval set, same metrics, Solar call 0, public report 생성 | Medium | experiment artifact 제외 |
| HD-PORTFOLIO-001 | HD-ROUTER-001 | README 결과 표와 포트폴리오 메시지 정리 | 채택/기각/보류 표, 과장 claim 0 | Low | docs revert |
...
- query type classifier가 아직 구현되지 않았으므로 실제 API routing 완성으로 표현하면 안 된다.
