...
| 기본 TTS 후보 | browser/managed TTS 중심 benchmark 후보 | 현재 실행 baseline은 Windows SAPI fallback, 다음 target은 `Piper` local TTS로 분리 |
...
| `local_piper` | TTS | 로컬 실행, API 비용 없음 | `rhasspy/piper` 계열은 빠른 local TTS target이나 Korean voice/license 확인 필요 | next target |
