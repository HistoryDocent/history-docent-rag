...
| `local_sherpa_onnx_supertonic3_ko` | TTS | 로컬 실행, API 비용 없음 | sherpa-onnx Supertonic 3 Korean smoke에서 5개 public-safe script private wav 합성 완료. 자동 metric, 청취 rubric, private score template, collection workflow, entry tool, completion verifier, private HTML score sheet는 준비됐지만 사람 청취 점수는 0건 | manual score input pending |
