...
기존 Azure gate는 실행을 차단한 증거로 유지하고, 기본 전략은 무료 로컬 STT/TTS로 바꾸는 것이 타당하다. local runtime install retry 기준 MeloTTS는 아직 Korean synthesis blocker가 있으므로 최종 provider로 확정하지 않는다. 다만 Windows SAPI Korean fallback은 실제 private wav 생성이 가능하므로 다음 단계는 managed provider가 아니라 local STT/TTS adapter integration이다.
