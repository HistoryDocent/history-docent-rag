...
추가 실행 결과: `HD-VOICE-STT-TTS-LOCAL-RUNTIME-MATRIX-001`에서 무료 로컬 후보 5개를 preflight했다. 현재 import 가능한 후보는 기존 `openai-whisper` fallback 1개뿐이며, `faster-whisper`, `MeloTTS`, `sherpa-onnx`, `piper` 계열은 미설치다. 설치, 모델 다운로드, STT/TTS 실행, 외부 provider 호출은 모두 0으로 유지했다.
...
기존 Azure gate는 실행을 차단한 증거로 유지하고, 기본 전략은 무료 로컬 STT/TTS로 바꾸는 것이 타당하다. local runtime matrix 기준 TTS 후보가 모두 미설치이므로 다음 단계는 managed provider가 아니라 local TTS runtime 후보 설치와 재실행이다.
