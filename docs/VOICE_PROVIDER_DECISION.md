...
| 기본 TTS 후보 | browser/managed TTS 중심 benchmark 후보 | `sherpa-onnx Supertonic 3 Korean`은 demo review 후보로 수락, production final provider는 미확정 |
...
| `local_sherpa_onnx_supertonic3_ko` | TTS | 로컬 실행, API 비용 없음 | 5개 public-safe script private wav 합성 완료, 자동 audio sanity 통과, local STT round-trip proxy 4/5, 사람 청취 점수 30/30 평균 5.0 | demo review candidate, final provider 미확정 |
...
| 음성 ML | STT는 `faster-whisper small CUDA`를 현재 demo evidence 후보로 유지한다. TTS는 `sherpa-onnx Supertonic 3 Korean`을 demo review 후보로 두고, 최종 provider 판단은 별도 production gate로 분리한다. |
...
| `primary_local_tts_candidate_count` | 0 | production final provider 기준 TTS 후보는 아직 없음 |
| `tts_demo_candidate_count` | 1 | `local_sherpa_onnx_supertonic3_ko` |
| `tts_human_score_completed_count` | 30 | private 사람 청취 점수 입력 완료 |
| `tts_human_score_overall_avg` | 5.000000 | 1인 청취 기준 평균 |
| `tts_human_score_provider_decision` | `candidate_accepted_for_demo_review` | demo review 후보 수락 |
...
| `tts_human_listening_completed_count` | 30 | 5개 script x 6 rubric 점수 완료 |
...
| `scope` | local STT/TTS demo 후보를 실제 demo flow에서 한 번에 확인하는 playback smoke를 만든다. |
| `acceptance_tests` | local STT candidate 1, TTS demo candidate 1, external provider call 0, external audio transmission 0, raw audio public artifact 0, production claim 0 |
| `risk_level` | Medium |
...
추가 실행 결과: `HD-VOICE-LOCAL-TTS-HUMAN-SCORE-FILL-001`에서 5개 script x 6개 기준 private score template과 public aggregate runner를 만들었다. 이후 completed score 30건, pending score 0건으로 갱신했고, public에는 criterion aggregate만 공개한다.

추가 실행 결과: `HD-VOICE-LOCAL-TTS-HUMAN-SCORE-COLLECTION-001`에서 private wav 5개 청취 평가 manifest와 guide를 만들었다. 이후 completed score 30건, pending score 0건으로 갱신했고, external provider 호출과 외부 음성 전송은 0건이다.

추가 실행 결과: `HD-VOICE-LOCAL-TTS-HUMAN-SCORE-ENTRY-001`에서 private score entry guide와 30행 draft를 만들었다. 이후 completed score 30건, pending score 0건으로 갱신했고, 개별 reviewer score detail은 public에 공개하지 않는다.

추가 실행 결과: `HD-VOICE-LOCAL-TTS-HUMAN-SCORE-ENTRY-COMPLETION-001`에서 private score 입력 완료 여부를 검증했다. completed score 30건, pending score 0건, public aggregate row 6건, external provider 호출 0건, 외부 음성 전송 0건이며 현재 decision은 `completed_human_scores_ready_for_provider_decision`이다.

추가 실행 결과: `HD-VOICE-LOCAL-TTS-HUMAN-SCORE-MANUAL-SCORING-001`에서 private HTML score sheet와 30행 draft를 만들었다. 이후 completed score 30건, pending score 0건으로 갱신했고, external provider 호출과 외부 음성 전송은 0건이다.

추가 실행 결과: `HD-VOICE-LOCAL-TTS-HUMAN-SCORE-DECISION-001`에서 사람 청취 점수 기반 provider decision gate를 실행했다. completed score 30건, pending score 0건, provider decision은 `candidate_accepted_for_demo_review`이며, external provider 호출과 외부 음성 전송은 0건이다. 이 결과는 demo review 후보 수락이며 최종 provider 확정이나 production 품질 보증으로 보지는 않는다.

추가 실행 결과: `HD-VOICE-LOCAL-TTS-HUMAN-SCORE-MANUAL-SCORING-RUNBOOK-001`에서 사람 청취 수동 채점 실행 절차를 갱신했다. private score sheet available 1건, private audio available 5건, completed score 30건, pending score 0건, user action required 0건, external provider 호출 0건, 외부 음성 전송 0건이다. 개별 점수 detail은 public artifact에 공개하지 않는다.

추가 실행 결과: `HD-VOICE-LOCAL-FREE-STACK-LOCK-001`에서 무료 로컬 음성 스택 기준을 제품 계약으로 고정했다. 당시 기준으로 `faster-whisper small CUDA`는 primary STT 후보 1건으로 고정하고, primary TTS 후보는 0건으로 유지했다. 이후 사람 청취 점수 입력 결과는 별도 `HD-VOICE-DEMO-STACK-DECISION-001` layer에서 반영한다.
...
추가 실행 결과: `HD-VOICE-DEMO-STACK-DECISION-001`에서 최신 사람 청취 점수를 반영해 무료 로컬 음성 데모 스택을 정리했다. STT는 `local_faster_whisper_small_cuda`, TTS demo review 후보는 `local_sherpa_onnx_supertonic3_ko`이며, TTS final provider count는 0건으로 유지한다. external provider 호출, 외부 음성 전송, raw audio public artifact, human score public detail row는 모두 0건이다.
