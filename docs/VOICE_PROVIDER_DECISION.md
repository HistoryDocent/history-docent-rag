...
| --- | --- | --- | --- | --- |
| `local_faster_whisper_small_cuda` | STT | 로컬 실행, API 비용 없음 | Whisper는 MIT, faster-whisper는 CUDA 실행에 적합 | primary STT |
| `local_whispercpp_small_cuda` | STT | 로컬 실행, API 비용 없음 | C/C++ 배포 후보지만 현재 환경에는 `whisper-cli` runtime과 `ggml` model file이 없음 | blocked deployment smoke |
...
| `local_sherpa_onnx_supertonic3_ko` | TTS | 로컬 실행, API 비용 없음 | sherpa-onnx Supertonic 3 Korean smoke에서 5개 public-safe script private wav 합성 완료. 자동 audio sanity는 통과했지만 local STT round-trip proxy는 4/5만 threshold를 통과했고 사람 청취 점수는 0건 | blocked missing human scores |
...
| `whisper_cpp` | whisper.cpp는 C/C++ 기반 local inference와 `whisper-cli` 실행 방식을 제공 | https://github.com/ggml-org/whisper.cpp |
...
| `primary_local_stt_candidate_count` | 1 | `local_faster_whisper_small_cuda` |
| `primary_local_tts_candidate_count` | 0 | 사람 청취 점수 기반 provider decision gate에서 아직 채택된 TTS 후보 없음 |
...
| `tts_automated_proxy_pass_count` | 4 | 5개 중 4개만 자동 proxy threshold 통과 |
| `tts_human_listening_completed_count` | 0 | 사람 청취 점수는 아직 없음 |
| `local_free_stack_lock_document_count` | 1 | 무료 로컬 음성 스택 기준 고정 문서 |
| `primary_local_stt_locked_count` | 1 | `faster-whisper small CUDA`를 현재 demo evidence 기준 STT 후보로 고정 |
| `local_tts_final_provider_claim_count` | 0 | 최종 TTS provider claim 금지 |
...
추가 실행 결과: `HD-VOICE-LOCAL-WHISPERCPP-DEPLOYMENT-SMOKE-001`에서 `whisper.cpp` local STT 배포성 smoke를 추가했다. 현재 환경은 CUDA를 감지했지만 `whisper-cli` runtime과 `ggml` model file이 없어 local STT 실행은 0건이다. external provider 호출과 외부 음성 전송은 0건이며, 기본 STT evidence 후보는 `local_faster_whisper_small_cuda`로 유지한다.
...
추가 실행 결과: `HD-VOICE-LOCAL-TTS-QUALITY-LISTENING-REVIEW-001`에서 sherpa-onnx private wav 5개의 duration, RMS, clipping, silence, sample rate 자동 metric과 6개 기준 human listening rubric을 기록했다. audio file available 5건, automated metric pass 5건, human listening completed 0건, external provider 호출 0건, 외부 음성 전송 0건이다. 이 결과는 품질 검증 완료가 아니라 사람 청취 평가를 위한 기준 고정이다.

추가 실행 결과: `HD-VOICE-LOCAL-TTS-HUMAN-SCORE-FILL-001`에서 5개 script x 6개 기준 private score template과 public aggregate runner를 만들었다. private template row 30건, completed score 0건, pending score 30건, public aggregate row 6건, external provider 호출 0건, 외부 음성 전송 0건이다. 이 결과는 점수 입력 framework이며 실제 사람 청취 품질 검증 완료가 아니다.

추가 실행 결과: `HD-VOICE-LOCAL-TTS-HUMAN-SCORE-COLLECTION-001`에서 private wav 5개 청취 평가 manifest와 guide를 만들었다. private audio available 5건, manifest row 5건, completed score 0건, pending score 30건, public aggregate row 6건, external provider 호출 0건, 외부 음성 전송 0건이다. 이 결과는 collection-ready 상태이며 실제 사람 청취 품질 검증 완료가 아니다.

추가 실행 결과: `HD-VOICE-LOCAL-TTS-HUMAN-SCORE-ENTRY-001`에서 private score entry guide와 30행 draft를 만들었다. private audio available 5건, draft row 30건, completed score 0건, pending score 30건, public aggregate row 6건, external provider 호출 0건, 외부 음성 전송 0건이다. 이 결과는 entry tool 준비 상태이며 실제 사람 청취 품질 검증 완료가 아니다.

추가 실행 결과: `HD-VOICE-LOCAL-TTS-HUMAN-SCORE-ENTRY-COMPLETION-001`에서 private score 입력 완료 여부를 검증했다. completed score 0건, pending score 30건, public aggregate row 6건, external provider 호출 0건, 외부 음성 전송 0건이며 현재 decision은 `blocked_missing_human_scores`다. 이 결과는 completion verifier 준비 상태이며 실제 사람 청취 품질 검증 완료가 아니다.

추가 실행 결과: `HD-VOICE-LOCAL-TTS-HUMAN-SCORE-MANUAL-SCORING-001`에서 private HTML score sheet와 30행 draft를 만들었다. score sheet created 1건, completed score 0건, pending score 30건, public aggregate row 6건, external provider 호출 0건, 외부 음성 전송 0건이며 현재 decision은 `ready_for_human_manual_scoring`이다. 이 결과는 수동 채점 workspace 준비 상태이며 실제 사람 청취 품질 검증 완료가 아니다.

추가 실행 결과: `HD-VOICE-LOCAL-TTS-HUMAN-SCORE-DECISION-001`에서 사람 청취 점수 기반 provider decision gate를 추가했다. 현재 completed score 0건, pending score 30건, provider decision은 `blocked_missing_human_scores`이며, external provider 호출과 외부 음성 전송은 0건이다. 사람 점수를 임의 생성하지 않았으므로 `local_sherpa_onnx_supertonic3_ko`는 아직 최종 TTS provider가 아니다.

추가 실행 결과: `HD-VOICE-LOCAL-TTS-HUMAN-SCORE-MANUAL-SCORING-RUNBOOK-001`에서 사람 청취 수동 채점 실행 절차를 고정했다. private score sheet available 1건, private audio available 5건, completed score 0건, pending score 30건, user action required 1건, external provider 호출 0건, 외부 음성 전송 0건이며 현재 decision은 `ready_for_manual_score_input`이다. 이 결과는 채점 실행 절차 준비 상태이며 실제 사람 청취 품질 검증 완료가 아니다.

추가 실행 결과: `HD-VOICE-LOCAL-FREE-STACK-LOCK-001`에서 무료 로컬 음성 스택 기준을 제품 계약으로 고정했다. `faster-whisper small CUDA`는 primary STT 후보 1건으로 고정하고, primary TTS 후보는 0건으로 유지한다. `sherpa-onnx Supertonic 3 Korean`은 experimental TTS 후보이며 사람 청취 점수 0건과 proxy threshold 4/5 결과 때문에 final provider로 채택하지 않는다. external provider 호출과 외부 음성 전송은 0건이다.

추가 실행 결과: `HD-VOICE-LOCAL-RUNTIME-STACK-ALIGN-001`에서 stack lock의 `local_faster_whisper_small_cuda`를 실제 runtime/API provider id와 정렬했다. provider id mismatch 0건, primary local STT 1건, primary local TTS 0건, TTS final provider 0건, external provider 호출 0건이며 TTS fallback은 `fallback_not_quality_candidate`로만 노출한다.
...
기존 Azure gate는 실행을 차단한 증거로 유지하고, 기본 전략은 무료 로컬 STT/TTS로 바꾸는 것이 타당하다. local runtime install retry 기준 MeloTTS는 아직 Korean synthesis blocker가 있으므로 최종 provider로 확정하지 않는다. Windows SAPI Korean fallback은 30개 local voice E2E regression과 local-only runtime contract까지 연결됐지만 production voice app 완성이나 최종 STT/TTS 품질 검증으로 표현하지 않는다. Bench v2 기준으로는 `faster-whisper`와 `Piper`를 다음 실행 target으로 분리했고, faster-whisper STT 비교 기준으로는 현재 evidence 후보를 `local_faster_whisper_small_cuda`로 올린다. whisper.cpp deployment smoke 기준으로는 runtime/model 부재가 blocker라서 기본 STT 후보를 변경하지 않는다. Piper smoke 기준으로는 runtime은 가능하지만 Korean voice 0개이므로 한국어 TTS 기본 provider에서 제외한다. Korean TTS alternative review 이후 `local_sherpa_onnx_supertonic3_ko`는 실제 private wav smoke와 자동 audio sanity metric까지 통과했고, local STT round-trip 자동 proxy는 5건 모두 실행됐지만 threshold pass는 4건이다. human score fill/collection/entry/completion verifier/manual scoring workspace/provider decision gate/manual scoring runbook도 만들었지만 사람 청취 점수는 아직 0건이다. local free stack lock 기준으로는 `faster-whisper small CUDA`만 primary STT 후보로 고정하고 TTS final provider는 0건으로 유지한다. runtime stack alignment 기준으로 실제 runtime/API provider id도 `local_faster_whisper_small_cuda`와 `fallback_not_quality_candidate` 상태로 정렬했다. 따라서 TTS 후보 채택은 `blocked_missing_human_scores`로 차단하며, STT/TTS 최종 provider 확정과 실제 관광객 음성 품질 검증 claim은 계속 금지한다.
