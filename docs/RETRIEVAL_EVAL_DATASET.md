...
| private dev dataset | `<private retrieval eval dev dataset>` |
| private test dataset | `<private retrieval eval test dataset>` |
...
| private test lock report | `evals/reports/retrieval_eval_private_test_lock_report.md` |
| private benchmark readiness report | `evals/reports/retrieval_eval_private_benchmark_readiness_report.md` |
...
| test lock report pipeline | `pipelines/build_retrieval_eval_test_lock_report.py` |
| benchmark readiness report pipeline | `pipelines/build_retrieval_eval_private_benchmark_report.py` |
...
| test lock and benchmark tests | `tests/test_retrieval_eval_test_and_benchmark_reports.py` |
...
- private: full dev/test JSONL을 public repository 밖의 local private storage에 저장한다.
...
private dev 70개는 public repository에 원본 JSONL을 commit하지 않고 집계 report만 공개한다.
...
query type별 reviewed 항목은 각각 dev 10개다.

| query_type | dev | draft | reviewed | target_dev | dev_shortfall |
| --- | ---: | ---: | ---: | ---: | ---: |
| `place_fact` | 10 | 0 | 10 | 10 | 0 |
| `place_story` | 10 | 0 | 10 | 10 | 0 |
| `relationship` | 10 | 0 | 10 | 10 | 0 |
| `overview` | 10 | 0 | 10 | 10 | 0 |
| `route_context` | 10 | 0 | 10 | 10 | 0 |
| `voice_followup` | 10 | 0 | 10 | 10 | 0 |
| `no_answer` | 10 | 0 | 10 | 10 | 0 |

## Private Test Lock Progress

private test 35개는 public repository에 원본 JSONL을 commit하지 않고 집계 report만 공개한다.

| 항목 | 값 |
| --- | --- |
| private dataset alias | `<private retrieval eval dataset: retrieval_eval_test.jsonl>` |
| public target report | `evals/reports/retrieval_eval_private_test_target_report.md` |
| public lock report | `evals/reports/retrieval_eval_private_test_lock_report.md` |
| review rubric | `docs/RETRIEVAL_EVAL_REVIEW_RUBRIC.md` |
...
query type별 locked 항목은 각각 test 5개다.

| query_type | test | draft | reviewed | locked | target_test | test_shortfall |
| --- | ---: | ---: | ---: | ---: | ---: | ---: |
...
private dev 70개와 private test 35개를 합친 full benchmark readiness gate를 통과했다.

| 항목 | 값 |
| --- | --- |
| public readiness report | `evals/reports/retrieval_eval_private_benchmark_readiness_report.md` |
...
현재 public dataset은 seed다. private full benchmark 105개는 private storage에서 관리하고 public에는 집계 report만 공개한다.

- 질문 수가 14개라 통계적 개선 주장을 할 수 없다.
- judgment는 1차 seed라 single-annotator 기준이다.
- private dev 70개와 locked test 35개는 준비됐지만 성능 개선 결과는 아직 없다.
- locked test는 최종 확인 전까지 튜닝 의사결정에 사용하지 않는다.
