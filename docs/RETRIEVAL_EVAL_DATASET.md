...
2. target resolvability gate를 통과한 항목만 `reviewed`로 승격한다.
3. private test 후보는 dev 튜닝 이후 query type별 5개씩 작성하고 `locked`로 고정한다.
4. test split은 최종 ablation 확인 전까지 튜닝에 사용하지 않는다.

## Private Dev Draft Progress

private dev 1차 draft는 public repository에 원본 JSONL을 commit하지 않고 집계 report만 공개한다.
...
query type별 1차 draft는 각각 dev 5개다.

| query_type | dev | target_dev | dev_shortfall |
| --- | ---: | ---: | ---: |
| `place_fact` | 5 | 10 | 5 |
| `place_story` | 5 | 10 | 5 |
| `relationship` | 5 | 10 | 5 |
| `overview` | 5 | 10 | 5 |
| `route_context` | 5 | 10 | 5 |
| `voice_followup` | 5 | 10 | 5 |
| `no_answer` | 5 | 10 | 5 |
...
현재 public dataset은 seed다. private dev 1차 draft는 작성됐지만 최종 성능 주장에는 아직 부족하다.

- 질문 수가 14개라 통계적 개선 주장을 할 수 없다.
- judgment는 1차 seed라 single-annotator 기준이다.
- private dev는 35개 draft 단계이며 dev 목표 70개와 test 목표 35개가 아직 남아 있다.
- private dev 1차 draft는 `review_status=draft`라 human review와 `reviewed` 승격이 필요하다.
