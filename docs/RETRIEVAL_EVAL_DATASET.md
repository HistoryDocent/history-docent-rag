...
현재 public dataset은 seed다. private dev 70개는 reviewed 상태지만 최종 성능 주장에는 아직 부족하다.

- 질문 수가 14개라 통계적 개선 주장을 할 수 없다.
- judgment는 1차 seed라 single-annotator 기준이다.
- private dev는 목표 70개를 채웠지만 test 목표 35개와 locked split이 아직 남아 있다.
- private dev review는 튜닝용 데이터 검수이므로 locked test 작성 전 추가 cross-check가 필요하다.
